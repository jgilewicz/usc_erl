...
def main(_):
    ...
    wandb.run.log_code(include_fn=lambda _: False)

    os.makedirs(save_dir, exist_ok=True)
    env = make_env(
        FLAGS.benchmark, FLAGS.env_name, FLAGS.seed, num_envs=FLAGS.num_seeds
    )
    eval_env = make_env(
        FLAGS.benchmark, FLAGS.env_name, FLAGS.seed + 42, num_envs=FLAGS.num_seeds
    )
    ...
    model_replay_buffer = ParallelReplayBuffer(
        env.observation_space,
        env.action_space.shape[-1],
        capacity=2000000,
        num_seeds=FLAGS.num_seeds,
    ...
    for i in tqdm.tqdm(
        ...
        if i % FLAGS.model_train_freq == 0 and FLAGS.use_model:
            agent.train_world_model_ensemble(
                replay_buffer,
                FLAGS.model_batch_size,
                FLAGS.model_num_updates,
        ...
        if FLAGS.use_model:
            model_batches = model_replay_buffer.sample_parallel_multibatch(
                FLAGS.batch_size, FLAGS.model_updates_per_step
            )
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            wandb.log(
                {
                    "total_steps": i,
                    **{
                        k: float(np.mean(aggregated_infos[k]))
                        for k in ("critic_loss", "actor_loss")
                        if k in aggregated_infos
                    },
                },
                step=i,
            )
            if i > 0:
                evaluate_agents_if_time(
                    ...
                    FLAGS.eval_interval,
                    FLAGS.eval_episodes,
                    save_video=getattr(FLAGS, "save_video", False),
                )
