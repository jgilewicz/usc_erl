...
def get_stable_final_values(env_id, merged_data):
    """
    Extracts stable final run values by averaging eval_reward
    ...
    for method in merged_data:
        ...
        for seed, df in merged_data[method].items():
            ...
            # Use the unified evaluation reward for all methods.
...
def run_significance_tests(env_id, stable_values, base_dir="results"):
    ...
    df_latex["p-value"] = df_latex["p-value"].apply(
    ...
    with open(latex_out, "w") as f:
        f.write(
            df_latex.style.hide(axis="index").to_latex(
                column_format="lcccc",
