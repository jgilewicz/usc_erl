...
def load_environment_data(env_id, base_dir='results'):
    ...
    """
    metrics = [
        'total_steps', 'eval_reward', 'rl_reward', 
        'best_population_fitness', 'avg_population_fitness', 
        'uncertainty_mean', 'uncertainty_max', 'uncertainty_threshold', 
        'generation'
    ]
    run_data = {}
...
def generate_sample_efficiency_plot(env_id, merged_data, out_path):
    ...
    for method in sorted(merged_data.keys()):
        ...
        for seed, df in merged_data[method].items():
            ...
            # Reward selection precedence logic
            ...
            interp_y = np.interp(step_grid, temp_df['total_steps'].values, temp_df[y_metric].values)
    ...
    ax.legend(loc='upper left', frameon=True, facecolor='white', framealpha=0.3, edgecolor='#f2f2f2')
    plt.tight_layout()
    ...
    for i, method in enumerate(surrogate_methods):
        ...
        for seed, df in merged_data[method].items():
            ...
            if not all((col in df.columns for col in required)):
                continue
            temp_df = df[required].dropna(subset=['generation', 'uncertainty_mean']).sort_values('generation')
            if temp_df.empty:
                continue
            mean_uncertainties.append(np.interp(gen_grid, temp_df['generation'].values, temp_df['uncertainty_mean'].values))
            avg_fitnesses.append(np.interp(gen_grid, temp_df['generation'].values, temp_df['avg_population_fitness'].values))
        ...
        line_mean, = ax.plot(gen_grid, avg_mean_u_smooth, color=color, linewidth=1.2, label='Uncertainty Mean')
        ...
        ax2.grid(False)
        line_fit, = ax2.plot(gen_grid, avg_fit_smooth, color='#de8f05', linewidth=1.0, linestyle='-.', label='Avg Pop Fitness')
        
        lines = [line_mean, line_fit]
        labels = [l.get_label() for l in lines]
        ax.legend(lines, labels, loc='upper left', frameon=True, facecolor='white', framealpha=0.3, edgecolor='#f2f2f2')
...
def generate_critic_correlation_plot(env_id, base_dir='results', out_base_dir=None):
    ...
    for method in methods:
        ...
        for col in df_loss.columns:
            match = re.match(rf'^{method}_{re.escape(env_id)}_seed(\d+) - critic_loss$', col)
            if match:
    ...
    for i, (method, (x, y)) in enumerate(valid_methods_data.items()):
        ...
        ax.legend(loc='upper right', frameon=True, facecolor='white', framealpha=0.3, edgecolor='#f2f2f2')
        sns.despine(ax=ax, top=True, right=True)
    ...
    with open(latex_out, 'w') as f:
        f.write(corr_df.style.hide(axis="index").to_latex(
            column_format='lccc', 
