...
def determine_method_from_run(run):
    ...
    if name == 'sc_erl':
        ...
        return name.lower()
    run_name = run.name
    for prefix in ['sc_erl_dropout', 'sc_erl_ensemble', 'sc_erl_random', 'td3', 'erl', 'ppo', 'ddpg']:
...
def determine_env_and_seed(run):
    ...
    if not env_id or seed is None:
        match = re.match('^(?:sc_erl_dropout|sc_erl_ensemble|sc_erl_random|td3|erl|ppo|ddpg)_([A-Za-z0-9\\-_]+)_seed(\\d+)', run_name)
        if match:
            if not env_id:
...
def main():
    ...
    METRICS = ['eval_reward', 'rl_reward', 'best_population_fitness', 'avg_population_fitness', 'uncertainty_mean', 'uncertainty_max', 'uncertainty_threshold', 'critic_loss', 'total_steps', 'generation']
    ...
    for run in runs:
        ...
        if not env_id or seed is None or (not method):
            print(f"Skipping run '{run.name}' (Unable to determine Env ID, Seed or Method).")
            continue
        ...
        if env_id not in env_groups:
            env_groups[env_id] = []
        env_groups[env_id].append({'run': run, 'seed': seed, 'method': method, 'env_id': env_id})
    print(f'Grouped runs into {len(env_groups)} environment(s): {list(env_groups.keys())}')
    for env_id, group in env_groups.items():
        ...
        for item in group:
            ...
            try:
                ...
                for metric in METRICS:
                    ...
                    history = history.rename(columns={'_step': 'Step', metric: col_name})
        ...
        for item in group:
            run = item['run']
            flat_config = flatten_dict({k: v for k, v in run.config.items() if not k.startswith('_')})
            all_config_keys.update(flat_config.keys())
            flat_summary = {k: v for k, v in run.summary._json_dict.items() if not k.startswith('_')}
            all_summary_keys.update(flat_summary.keys())
            preprocessed_runs.append({'run': run, 'flat_config': flat_config, 'flat_summary': flat_summary})
        ...
        for item in preprocessed_runs:
            ...
            row = {'Name': run.name, 'State': run.state, 'Notes': run.notes if run.notes else '-', 'User': run.user.username if run.user else '', 'Tags': str(run.tags), 'Created': run.created_at, 'Runtime': str(run.summary.get('_runtime', '')), 'Sweep': run.sweep.id if run.sweep else ''}
